...
@brief : A4 - Exploration and Fun!: Amazon S3 Glacier (Vaults) + Amazon SNS (Simple Notification Service); downloadArchiveResult.py

@note :
    Description: Gets the results of a retrieve archive job from the Glacier vault

    List Jobs:
    $ aws glacier list-jobs --account-id - --vault-name a4cis4010

    Describe Job:
    $ aws glacier describe-job --vault-name a4cis4010 --account-id - --job-id Djy3OWsa_X6E8FJ7811xMRbYBnhyaOhYHtqg1iLSvsNmdShzXzEdQ-Ndiwe6kjl6w1HAwWCKMOJ2AdrdsjoBq_dX2-Jb

    Get Job Output, Archive:
    $ aws glacier get-job-output --account-id - --vault-name a4cis4010 --job-id Djy3OWsa_X6E8FJ7811xMRbYBnhyaOhYHtqg1iLSvsNmdShzXzEdQ-Ndiwe6kjl6w1HAwWCKMOJ2AdrdsjoBq_dX2-Jb output.jpg
'''
...
USAGE_STATEMENT = "Usage: py downloadArchiveResult.py archive-retrieval-job-id"
...
def main():
    ...
    # MVB - testing file read-write, to .bin and back to .jpg
    # # Create filenames using the current date-time, but only keep alphanumeric chars
    # date_time_now_str = re.sub("[^a-zA-Z0-9]+", "", str(datetime.datetime.now()))
    # raw_filename = f"{date_time_now_str}.bin"
    # filename = f"{date_time_now_str}.jpg"

    # # Then write the retrieved archive as an object data binary stream to a raw .bin and normal .jpg file
    # try:
    #     my_fn = "demon-slayer_squirtle.jpg"
    #     with open(my_fn, 'rb') as f:
    #         object_data = f.read()
    #     f.close()
    ...
    #     sys.exit(f"[ERROR] While reading object data stream from file '{my_fn}' result to files '{raw_filename}' and '{filename}': {e}")
    #     return None
    ...
    # ========== RETRIEVE ARCHIVE JOB RESULTS ==========

    # Retrieve the job resulting filename (where the archive was stored) and description
    #new_filename, archive_description = retrieve_archive_result(VAULT_NAME, job_id_input)
    new_filename = retrieve_archive_result(VAULT_NAME, job_id_input)

    if new_filename is None:
        sys.exit(f"ERROR: Archive retrieval job results for Glacier vault '{VAULT_NAME}' job ID '{job_id_input}' failed.")
    print(f"Successfully retrieved archive job results for Glacier vault '{VAULT_NAME}' - Job ID Input: {job_id_input}")

    # archive description if present, newfile.bin and .jpg
    print(f"Downloaded file: (raw) '{new_filename}.bin' & '{new_filename}.jpg'") # with archive description: {archive_description}")

############################################ FUNCTIONS ############################################

# Gets the results of an inventory retrieval job
#   NVM - Returns the filename of the downloaded archive (without a file extension, so we can use it for either .bin or .jpg later)
#   Returns the description of the retrieved archive (if not present, may be None)
def retrieve_archive_result(vault_name, job_id):
    # Retrieve the job results
    try:
        response = glacier_client.get_job_output(vaultName=vault_name, jobId=job_id)
    except Exception as e:
        sys.exit(f"[ERROR] While getting archive retrieval job results: {e}")
    ...
    # Read the result of the description and object data binary stream into variables
    ...
    try:
        ...
        sys.exit(f"[ERROR] While reading object data stream result to files '{raw_filename}' and '{filename}': {e}")
        return None
    
    return date_time_now_str #, archive_description
